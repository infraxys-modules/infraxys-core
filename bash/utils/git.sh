...
function create_github_team() {
    ...
    import_args "$@";
    check_required_arguments "$function_name" github_domain github_token organization team_name team_description;
...
EOF
    ...
    curl -k -s -H "Authorization: token $github_token" \
        ...
        -X POST \
        --data "$data" \
        $github_rest_endpoint/orgs/$organization/teams
    
    log_info "Created GitHub team $team_name with description '$team_description' in organization '$organization"
}
