    ...
    if [ -z "${!variable_name}" ]; then
        if [ -n "$or_variable_name" ]; then
            if [ -z "${!or_variable_name}" ]; then
                log_error "Variable '$variable_name' or variable '$or_variable_name' is required. $function_name";
                exit 1;
            fi;
        else
        	if [ -z "$default_value" ]; then
        		log_error "Required variable '$variable_name' is not set and no default value is specified. $function_name";
    	    	exit 1;
        	else
